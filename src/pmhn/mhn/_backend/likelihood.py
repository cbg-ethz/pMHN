...
def f0(
    s: jnp.ndarray, p: jnp.ndarray, m: jnp.ndarray
...
def z1(s: jnp.ndarray) -> tuple[jnp.ndarray, float]:
    return s.reshape((-1, 2), order="C").flatten(order="F"), 0.0
...
def deriv_no_seed(
    ...
    z_sync, z_prim, z_met, d_th_i = lax.fori_loop(
        lower=0, upper=i, body_fun=body_fun, init_val=(z_sync, z_prim, z_met, d_th_i)
    )
...
def cond_p_obs(
    ...
    """
    compatible_states = obs_states(
        n_joint=n_joint, state=state_joint, pt_first=pt_first
    )
...
def loglikelihood_nonzero(
    ...
    p0 = jnp.zeros_like(n_prim_shape)
    p0 = p0.at[0].set(1.0)
...
def loglikelihood_zero(log_theta: jnp.ndarray) -> jnp.ndarray:
    ...
    """
    diag = jnp.diag(log_theta)
    log_mutation_rates = logsumexp(diag)
...
def grad_loglikelihood_zero(
    ...
    log_pth = jnp.log(jnp.array([1.0 / (1.0 + jnp.sum(theta_br))]))
...
def q_inv_deriv_pth(
    ...
    """
    q = R_i_inv_vec(
        log_theta, log_d_p, log_d_m, q, state_joint, n_joint, transpose=True
    )
...
def marginal_obs_pt_first(
    ...
    poss_states_inds = jnp.where(p == 1.0, size=2 ** (n_met - 1))[0]
...
def _g_coupled_0(
    ...
    pTh1_joint = R_i_inv_vec(
        log_theta, log_d_p, log_d_m, p, state_joint, n_joint, transpose=False
    )

    pf_exp_score, pf_g_1, pf_d_dp_1, pf_d_dm_1, pf_p = marginal_obs_pt_first(
        log_theta, log_d_p, log_d_m, pTh1_joint, state_joint, met, n_joint, n_met
    )
    mf_exp_score, mf_g_1, mf_d_dp_1, mf_d_dm_1, mf_p = marginal_obs_mt_first(
        log_theta, log_d_p, log_d_m, pTh1_joint, state_joint, prim, n_joint, n_prim
    )
    ...
    g_2, d_dp_2, d_dm_2 = q_inv_deriv_pth(
        log_theta, log_d_p, log_d_m, pf_p + mf_p, pTh1_joint, state_joint, n_joint
    )
...
def _g_coupled_1(
    ...
    exp_score, g_1, d_dp_1, d_dm_1, p = marginal_obs_pt_first(
        log_theta, log_d_p, log_d_m, pTh1_joint, state_joint, met, n_joint, n_met
    )
    # Derivative of pth1_cond
    p = diag_scal_p(log_d_p, state_joint, p)
    g_2, d_dp_2, d_dm_2 = q_inv_deriv_pth(
        log_theta, log_d_p, log_d_m, p, pTh1_joint, state_joint, n_joint
    )
    d_dm = d_dm_1 - d_dm_2
    d_dp = d_dp_1 - d_dp_2
...
def _g_coupled_2(
    ...
    exp_score, g_1, d_dp_1, d_dm_1, p = marginal_obs_mt_first(
        log_theta, log_d_p, log_d_m, pTh1_joint, state_joint, prim, n_joint, n_prim
    )
