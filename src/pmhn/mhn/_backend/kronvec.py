...
def diagnosis_theta(log_theta: jnp.ndarray, log_diag_rates: jnp.ndarray) -> jnp.ndarray:
    ...
    scaled_theta = jnp.apply_along_axis(
        lambda x, y: (x - y), 1, log_theta, log_diag_rates
    )
...
def k1d00(p: jnp.ndarray, theta: float = 1.0) -> jnp.ndarray:
    return p


def k2dt0(
    p: jnp.ndarray, theta: float, diag: bool = True, transpose: bool = True
) -> jnp.ndarray:
    p = p.reshape((-1, 2), order="C")
    theta_slice = jnp.array([-theta, 0.0])
...
def k2d0t(p: jnp.ndarray, theta: float) -> jnp.ndarray:
    p = p.reshape((-1, 2), order="C")
    theta_slice = jnp.array([0.0, theta])
    p = mult_t_x_vec(theta_slice, p)
    return p.flatten(order="F")
...
def k2d1t(p: jnp.ndarray, theta: float) -> jnp.ndarray:
    p = p.reshape((-1, 2), order="C")
    theta_slice = jnp.array([1.0, theta])
    p = mult_t_x_vec(theta_slice, p)
    return p.flatten(order="F")


def k2d10(p: jnp.ndarray, theta: float = 1.0) -> jnp.ndarray:
    p = p.reshape((-1, 2), order="C")
    theta_slice = jnp.array([1.0, 0.0])
    p = mult_t_x_vec(theta_slice, p)
    return p.flatten(order="F")
...
def k2d01(p: jnp.ndarray) -> jnp.ndarray:
    p = p.reshape((-1, 2), order="C")
    theta_slice = jnp.array([0.0, 1.0])
    p = mult_t_x_vec(theta_slice, p)
    return p.flatten(order="F")


def k2d11(p: jnp.ndarray, theta: float = 1.0) -> jnp.ndarray:
    p = p.reshape((-1, 2), order="C")
    return p.flatten(order="F")


def k2ntt(
    p: jnp.ndarray, theta: float, diag: bool = True, transpose: bool = False
) -> jnp.ndarray:
    t = lax.select_n(
        2 * diag + transpose,
        jnp.array([[0.0, theta], [0.0, 0.0]]),
        jnp.array([[0.0, 0.0], [theta, 0.0]]),
        jnp.array([[-theta, theta], [0.0, 0.0]]),
        jnp.array([[-theta, 0.0], [theta, 0.0]]),
    )
    p = p.reshape((-1, 2), order="C")
    p = p @ t
    return p.flatten(order="F")
...
def k4d100t(
    ...
    p = p.reshape((-1, 4), order="C")
    theta_slice = jnp.array([1.0, 0.0, 0.0, theta])
...
def k4dt000(
    ...
    p = p.reshape((-1, 4), order="C")
    theta_slice = jnp.array([-theta, 0.0, 0.0, 0.0])
    p = mult_t_x(theta_slice, p)
    return p.flatten(order="F")
...
def k4dtt00(
    ...
    p = p.reshape((-1, 4), order="C")
    theta_slice = jnp.array([-theta, -theta, 0.0, 0.0])
    p = mult_t_x(theta_slice, p)
    return p.flatten(order="F")
...
def k4dt0t0(
    ...
    p = p.reshape((-1, 4), order="C")
    theta_slice = jnp.array([-theta, 0.0, -theta, 0.0])
    p = mult_t_x(theta_slice, p)
    return p.flatten(order="F")
...
def k4d1t1t(p: jnp.ndarray, theta: float) -> jnp.ndarray:
    p = p.reshape((-1, 4), order="C")
    theta_slice = jnp.array([1.0, theta, 1.0, theta])
    p = mult_t_x(theta_slice, p)
    return p.flatten(order="F")
...
def k4d11tt(p: jnp.ndarray, theta: float) -> jnp.ndarray:
    p = p.reshape((-1, 4), order="C")
    theta_slice = jnp.array([1.0, 1.0, theta, theta])
    p = mult_t_x(theta_slice, p)
    return p.flatten(order="F")


def k1ns(p: jnp.ndarray, theta: float, diag: bool = True, transpose: bool = True):
    return -theta * p
...
def _kronvec_sync(
    ...
    p = lax.fori_loop(lower=0, upper=i, body_fun=loop_body_diag, init_val=p)
    ...
    # Diagonal Kronecker factors
    p = lax.fori_loop(
        lower=i + 1, upper=log_theta.shape[0] - 1, body_fun=loop_body_diag, init_val=p
    )

    # Last Kronecker factor
    p = lax.cond(state[-1] == 1, k2d10, k1d00, p, 0.0)
...
def kronvec_sync(
    ...
    """This computes the restricted version of the product of the
    synchronized part of the ith Q summand
    Q_i with a vector Q_i p.

    Args:
        log_theta (jnp.ndarray): Log values of the theta matrix
        p (jnp.ndarray): Vector to multiply with from the right.
            Length must equal the number of
            nonzero entries in the state vector.
        i (int): Index of the summand.
        state (jnp.ndarray): Binary state vector,
            representing the current sample's events.
        diag (bool, optional): Whether to use the diagonal of Q_i
            (and not set it to 0). Defaults to True.
...
def _kronvec_prim(
    ...
    # Diagonal Kronecker factors
    p = lax.fori_loop(lower=i + 1, upper=n, body_fun=loop_body_diag, init_val=p)
...
def _kronvec_seed(
    ...
    # Diagonal Kronecker factors
    p = lax.fori_loop(lower=0, upper=n, body_fun=loop_body_diag, init_val=p)

    # Non-diagonal Kronecker factor
    p = lax.cond(state[-1] == 1, k2ntt, k1ns, p, theta_i[n], diag, transpose)
...
def kronvec(
    ...
    def body_fun(i, val):
        val += kronvec_sync(
            log_theta=log_theta, p=p, i=i, state=state, diag=diag, transpose=transpose
        )
        val += kronvec_prim(
            log_theta=log_theta, p=p, i=i, state=state, diag=diag, transpose=transpose
        )
    ...
    n = log_theta.shape[0] - 1
    y = lax.fori_loop(lower=0, upper=n, body_fun=body_fun, init_val=jnp.zeros_like(p))

    y += kronvec_seed(
        log_theta=log_theta, p=p, state=state, diag=diag, transpose=transpose
    )
...
@jit
def diag_scal_p(
    log_d_p: jnp.ndarray, state: jnp.ndarray, p: jnp.ndarray
) -> jnp.ndarray:
    ...
    d_p = jnp.exp(log_d_p)
    # Diagonal Kronecker factors
    pt_d_rates = lax.fori_loop(lower=0, upper=n, body_fun=sync_loop, init_val=p)
...
def _partial_diag_scal_p(
    log_d_p: jnp.ndarray, state: jnp.ndarray, p: jnp.ndarray, i: int
):
    def sync_loop(j, val):
        val = lax.switch(
            state[2 * j] + 2 * state[2 * j + 1],
    ...
    pt_d_rates = lax.cond(
        state[2 * i] + state[2 * i + 1] == 1, k2d0t, k4d0t0t, pt_d_rates, d_p[i]
    )
    pt_d_rates = lax.fori_loop(
        lower=i + 1, upper=n, body_fun=sync_loop, init_val=pt_d_rates
    )
    pt_d_rates = k2d1t(pt_d_rates, d_p[-1])
    return pt_d_rates
...
@jit
def diag_scal_m(
    log_d_m: jnp.ndarray, state: jnp.ndarray, p: jnp.ndarray, i: int = 0
) -> jnp.ndarray:
    ...
    n = log_d_m.shape[0] - 1
    d_m = jnp.exp(log_d_m)
    mt_d_rates = lax.fori_loop(lower=0, upper=n, body_fun=met_loop, init_val=p)
    mt_d_rates = k2d0t(mt_d_rates, d_m[-1])
    return mt_d_rates


def _partial_diag_scal_m(
    log_d_m: jnp.ndarray, state: jnp.ndarray, p: jnp.ndarray, i: int
) -> jnp.ndarray:
    """Multiplies a vector p with the diagonal matrix of MT-diagnosis effects d_m
    ...
    mt_d_rates = lax.cond(
        state[2 * i] + state[2 * i + 1] == 1, k2d0t, k4d00tt, mt_d_rates, d_m[i]
    )
    mt_d_rates = lax.fori_loop(
        lower=i + 1, upper=n, body_fun=met_loop, init_val=mt_d_rates
    )
    return k2d0t(mt_d_rates, d_m[-1])


@jit
def partial_diag_scal_m(
    log_d_m: jnp.ndarray, state: jnp.ndarray, p: jnp.ndarray, i: int
) -> jnp.ndarray:
    n = log_d_m.shape[0] - 1
    return lax.switch(
        (state[jnp.min(jnp.array([2 * n, 2 * i + 1]))]) + (i == n),
...
def kron_sync_diag(
    log_theta: jnp.ndarray, i: int, state: jnp.ndarray, n_state: int
) -> jnp.ndarray:
    ...
    diag = lax.fori_loop(lower=0, upper=i, body_fun=loop_body, init_val=diag)
    ...
    diag = lax.fori_loop(lower=i + 1, upper=n, body_fun=loop_body, init_val=diag)

    diag = lax.cond(state[-1] == 1, k2d10, k1d00, diag, 1.0)
...
def kron_prim_diag(
    ...
    return lax.cond(
        state[-1] == 0, diag_zero_fun, _kron_prim_diag, log_theta, p, i, state
    )
...
def kron_met_diag(
    ...
    diag = jnp.ones(2**n_state)
    return lax.cond(
        state[-1] == 0, diag_zero_fun, _kron_met_diag, log_theta, diag, i, state
    )


def kron_seed_diag(
    log_theta: jnp.ndarray, state: jnp.ndarray, n_state: int
) -> jnp.ndarray:
    ...
    theta_i = jnp.exp(log_theta[n, :])
    # Diagonal Kronecker factors
    diag = lax.fori_loop(lower=0, upper=n, body_fun=loop_body, init_val=diag)

    # Last Kronecker factor
    diag = lax.cond(state[-1] == 1, k2dt0, k1ns, diag, theta_i[n])
...
def kron_diag(log_theta: jnp.ndarray, state: jnp.ndarray, n_state: int) -> jnp.ndarray:
    ...
    def body_fun(i, val):
        val += kron_sync_diag(log_theta=log_theta, i=i, state=state, n_state=n_state)
        val += kron_prim_diag(log_theta=log_theta, i=i, state=state, n_state=n_state)
        val += kron_met_diag(log_theta=log_theta, i=i, state=state, n_state=n_state)

        return val

    n = log_theta.shape[0] - 1
    y = jnp.zeros(2**n_state)
    y = lax.fori_loop(lower=0, upper=n, body_fun=body_fun, init_val=y)
...
def obs_states(n_joint: int, state: jnp.ndarray, pt_first: bool = True) -> jnp.ndarray:
    ...
    p = lax.cond(state[-1] == 0, keep_all, keep_col2, p)
    return p
