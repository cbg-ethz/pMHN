...
def k1dt(
    p: jnp.ndarray, theta: jnp.ndarray, diag: bool = True, transpose: bool = True
) -> jnp.ndarray:
    return -theta * p
...
def _kronvec(
    ...
    def loop_body_diag(j, val):
        val = lax.cond(state[j] == 0, k1d1, k2d1t, val, theta_i[j])
        return val
    ...
    p = lax.cond(state[i] == 0, k1dt, k2ntt, p, theta_i[i], diag, transpose)
...
def kronvec(
    ...
    n = log_theta.shape[0]
    return jnp.sum(
        vmap(kronvec_i, (None, None, 0, None, None, None), 0)(
            log_theta, p, jnp.arange(n, dtype=int), state, diag, transpose
        ),
        0,
    )
...
def _scal_p_d(
    x: tuple[jnp.ndarray, jnp.ndarray], d_p: jnp.ndarray, d_m: jnp.ndarray
...
@jit
def scal_d_pt(
    log_d_p: jnp.ndarray, log_d_m: jnp.ndarray, state: jnp.ndarray, vec: jnp.ndarray
) -> tuple[jnp.ndarray, jnp.ndarray]:
    def loop_body_diag(j, val):
        val = lax.cond(state[j] == 1, _scal_p_d, _scal_p_1, val, d_p[j], d_m[j])
        return val
    ...
    d_m = jnp.exp(log_d_m)
    p = lax.fori_loop(
        lower=0, upper=n - 1, body_fun=loop_body_diag, init_val=(vec, vec)
    )
...
def _d_scal_d_pt(
    ...
    d_m = jnp.exp(log_d_m)
    dp_dd = lax.fori_loop(
        lower=0, upper=i, body_fun=loop_body_diag, init_val=(vec, vec)
    )

    dp_dd = (k2d0t(dp_dd[0], d_p[i]), k2d0t(dp_dd[1], d_m[i]))

    dp_dd = lax.fori_loop(lower=i + 1, upper=n, body_fun=loop_body_diag, init_val=dp_dd)
...
def x_partial_D_y(
    ...
    n = log_d_p.shape[0]
    res = vmap(body_fun, (0, None, None, None, None, None), 0)(
        jnp.arange(n), log_d_p, log_d_m, state, x, y
    )
...
def kron_diag_i(
    log_theta: jnp.ndarray, i: int, state: jnp.ndarray, diag: jnp.ndarray
) -> jnp.ndarray:
    def loop_body(j, val):
        val = lax.cond(state[j] == 0, k1d1, k2d1t, val, theta_i[j])
        return val
    ...
    diag = lax.cond(state[i] == 0, k1dt, k2dt0, diag, theta_i[i])
...
@jit
def kron_diag(
    log_theta: jnp.ndarray, state: jnp.ndarray, diag: jnp.ndarray
) -> jnp.ndarray:
    # def body_fun(i, val):
    ...
    n = log_theta.shape[0]
    return jnp.sum(
        vmap(kron_diag_i, (None, 0, None, None), 0)(
            log_theta, jnp.arange(n), state, diag
        ),
        axis=0,
    )
    # return lax.fori_loop(
...
def R_inv_vec(
    ...
    lidg = -1 / (
        kron_diag(log_theta=log_theta, state=state, diag=jnp.ones_like(x)) - d_rates
    )
    y = lidg * x

    y = lax.fori_loop(lower=0, upper=state_size + 1, body_fun=body_fun, init_val=y)
...
def x_partial_Q_y(
    ...
    def body_fun(i, val):
        def inner_fun(j, val):
            z, _val = lax.cond(state[j] == 0, t_x_0, f, val[0])
            return z, val[1].at[j].set(_val)

        z = x * kronvec_i(log_theta=log_theta, p=y, i=i, state=state)

        z, val = lax.fori_loop(lower=0, upper=i, body_fun=inner_fun, init_val=(z, val))

        z, _val = lax.cond(state[i] == 0, t_x_sum, t, z)

        val = val.at[i].set(_val)

        z, val = lax.fori_loop(
            lower=i + 1, upper=n, body_fun=inner_fun, init_val=(z, val)
        )
...
@jit
def gradient(
    log_theta: jnp.ndarray, state: jnp.ndarray, p_0: jnp.ndarray
) -> jnp.ndarray:
