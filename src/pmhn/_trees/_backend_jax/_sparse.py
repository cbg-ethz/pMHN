...
class COOMatrix(NamedTuple):
    ...
    def to_dense(self) -> Float[Array, "size size"]:
        ...
        def _diag_loop_body(i: int, a: Float[Array, "size size"]) -> Float[Array, "size size"]:
        ...
        def _offdiag_loop_body(i: int, a: Float[Array, "size size"]) -> Float[Array, "size size"]:
            return a.at[self.offdiagonal.start[i], self.offdiagonal.end[i]].set(self.offdiagonal.value[i])
