...
def test_generate_valid_tree():
    ...
    theta = np.array([[-1.41, 0.00, 0.00, 4.91, 1.03, 0.00, -1.91, -0.74, -1.35, 1.48],
 ...
 [0.97, 1.75, 0.00, -3.66, -1.28, 0.00, 1.66, 0.00, 0.00, -3.03]]
    ...
    for _ in range(10000):
        tree = simulate.generate_valid_tree(rng, theta, sampling_time, min_tree_size, max_tree_size)
...
def test_generate_tree_no_size_constraints():
 ...
 [0.97, 1.75, 0.00, -3.66, -1.28, 0.00, 1.66, 0.00, 0.00, -3.03]]
    ...
    for _ in range(10000):
        tree = simulate.generate_valid_tree(rng, theta, sampling_time)
...
def test_generate_tree_no_min_size_constraint():
 ...
 [0.97, 1.75, 0.00, -3.66, -1.28, 0.00, 1.66, 0.00, 0.00, -3.03]]
    ...
    for _ in range(10000):
        tree = simulate.generate_valid_tree(rng, theta, sampling_time, max_tree_size=max_tree_size)
...
def test_generate_tree_no_max_size_constraint():
 ...
 [0.97, 1.75, 0.00, -3.66, -1.28, 0.00, 1.66, 0.00, 0.00, -3.03]]
    ...
    for _ in range(10000):
        tree = simulate.generate_valid_tree(rng, theta, sampling_time, min_tree_size=min_tree_size)
...
def test_find_possible_mutations_except_positive(): 
    ...
    assert str(excinfo.value) == "Invalid mutation 212 in old_mutations. It should be 0 <= mutation <= 10."
...
def test_find_possible_mutations_except_negative():    
    ...
    assert str(excinfo.value) == "Invalid mutation -23 in old_mutations. It should be 0 <= mutation <= 10."
