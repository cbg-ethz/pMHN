...
df_10000 = pd.read_csv("likelihood_R/trees_10000_R.csv")
# theta matrices
theta_AML = csv_to_numpy("likelihood_R/MHN_Matrix_AML.csv")
theta_500 = csv_to_numpy("likelihood_R/MHN_Matrix_500.csv")
theta_10000 = csv_to_numpy("likelihood_R/MHN_Matrix_10000.csv")
# loglikelihoods in R
...
for idx, tree in trees_10000.items():
    print(f"Processing tree {idx} of {len(trees_10000)}")
    tree_log = LoglikelihoodSingleTree(tree)
    log_value = backend.loglikelihood(tree_log, theta_10000, sampling_rate)
    log_vec_py_10000[idx - 1] = log_value
    print(f"log_value: {log_value}")
...
np.savetxt("likelihood_py/log_vec_py_10000.csv", log_vec_py_10000, delimiter=",")
...
if np.allclose(log_vec_py_10000, log_vec_R_10000, atol=1e-10):
    print(
        "The loglikelihoods of the 10000 randomly generated"
        " trees are the same in R and Python."
    )
